    ...
    return (
        <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <Typography variant="h4" color={"white"} mt={4}>Distillery Data</Typography>
                <Button variant="contained" color="primary" onClick={handleAddDistilleryClick}>Add Distillery</Button>
            </div>
            <Grid container spacing={2} mt={2}>
                {distilleries.length > 0 ? (
                    distilleries.map((distillery, index) => (
                        <Grid item xs={12} sm={6} md={4} key={index}>
                            <Card sx={{ cursor: 'pointer' }} onClick={() => handleDistilleryClick(distillery?.id)}>
                                <CardMedia
                                    component="img"
                                    height="140"
